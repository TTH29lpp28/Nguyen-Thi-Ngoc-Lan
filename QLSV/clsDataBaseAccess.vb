...
Public Class DataBaseAccess
    ...
    Private ConnectionString As String = "Data Source=DESKTOP-4H8HO6V;Initial Catalog=DA7;Integrated Security=True;TrustServerCertificate=True"
    ...
    Public Function LayDiem(ByVal Cot As String, ByVal Bang As String, ByVal TableName As String, ByVal Masinhvien As String) As String

        Dim dtable As New DataTable(TableName)
        Dim Dl As String
        SqlDa = New SqlDataAdapter("Select " & Cot & " from " & Bang & " Where MaSV = " & Masinhvien, SqlCon)
        If OpenConnection() Then
            Try
                SqlDa.Fill(dtable)
                Dl = dtable.Rows(0)(Cot).ToString()
            Catch ex As SqlException
                MsgBox(ex.Message, MsgBoxStyle.Critical, "Get DataTable")
            Finally
                ' Close database connection.
